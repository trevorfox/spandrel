...
Quality infrastructure release. Adds the full advisory audit stack (cheap heuristics → wiring → CLI → priority queue → collection schemas → detector noise reduction → task-fidelity harness), Phase E1 semantic infrastructure (embedding store + missing-link detection with a zero-setup local provider), and context-pack hygiene on the MCP wire surface. The throughline: shift from "the compiler tells you what's broken" to "the framework tells you what's *low signal* and helps you measure whether fixing it makes agents better at real tasks." Public TypeScript surface is unchanged; new CLI subcommands (`spandrel audit`, `spandrel embed`) are additive; MCP tool responses now omit null/empty fields. REST and typed consumers see no schema change.

### Changed

- **MCP tool responses strip null/undefined/empty-string fields at the wire boundary.** A single `stripNulls` pass at `src/server/mcp.ts` `asTextResult` walks every JSON tool response before serialization and omits keys whose value is `null`, `undefined`, or `""`. `false`, `0`, empty arrays, and empty objects are preserved (those carry meaning). Non-plain objects (`Date`, `Map`, class instances) pass through untouched so `JSON.stringify` can apply its own serialization rules. **REST API responses and TypeScript types are unchanged** — typed REST consumers (Cannon, downstream TS clients) keep their stable schema with explicit nulls; the hygiene rule is a wire-boundary transform on the MCP surface only, so MCP and REST emit different shapes for the same underlying data. Consumers reading parsed MCP JSON with `?? fallback` or `key in obj` are unaffected; consumers that read `field === null` as a structured signal would need to switch to absence checks. **Savings depend on graph maturity.** On *bootstrap / discovery-stage* graphs (most edges undescribed, no git history yet), hygiene's impact is high — the case the change targets directly. On *mature / audited* graphs (load-bearing edge descriptions filled in, git history present), most of the nulls hygiene would strip have already been replaced with authored values; the remaining structural wins (root-node `parent`, content-omitted `content`, history-less `created`/`updated`/`author`) are modest. Measured calibration on three mature graphs (Spandrel `docs/`, EA-OS, Definite): wire-size reduction is **0–12% per tool, 0.6–2.0% in total**, dominated by `navigate` and `get_node` metadata nulls; `get_references` and `get_graph` see ~0% because the audited graphs have no nulls left to strip there. The freshly-bootstrapped state where hygiene helps most is not measured here. See `specs/2026-05-11-context-pack-hygiene.md` "Calibration findings" for the full breakdown. Cannon coordination: zero — Cannon doesn't typed-consume MCP wire output internally.

### Added

- **Phase E1 — embedding infrastructure + missing-link detection.** New `spandrel embed` subcommand populates a per-graph SQLite cache of node embeddings (`<graph-root>/_audit/embeddings.db`, gitignored). New `spandrel audit --semantic` flag reads that cache and runs the missing-link detector — surfaces node pairs that are semantically close (cosine similarity ≥ 0.75 by default) but graph-distant (no `link`-type edge connects them; hierarchy edges don't count). Findings emit as a new `missing_link` warning type with messages of the form `[missing_link] Considered linking to <target> (cos 0.NN)`. **Three embedding providers ship**, with the JS-native `local` provider as the zero-setup default: `local` (`fastembed` + `Xenova/all-MiniLM-L6-v2`, 384 dim, in-process ONNX runtime; downloads a ~25MB model file to `~/.cache/spandrel/embeddings/` on first use and caches it; no API key, no service), `openai` (`text-embedding-3-small`, requires `OPENAI_API_KEY`), and `ollama` (`nomic-embed-text`, requires a running Ollama server; `OLLAMA_HOST` configurable). OpenAI and Ollama are explicit opt-ins (`--provider openai|ollama`); the default never auto-selects OpenAI when `OPENAI_API_KEY` is set, to avoid surprising users with API charges they didn't ask for. The store is content-addressed by `(path, sha256(name+description+body), model)` so re-runs of `spandrel embed` are idempotent — only nodes whose content has changed get re-embedded. `spandrel audit --semantic` errors with a clear hint pointing at `spandrel embed` when the store is missing or stale, and **auto-detects the model from the store** when only one is present (multiple → asks the user to pass `--semantic-model <name>`). v1 emits symmetric candidates (both directions when both pass threshold); a suppression mechanism and LLM-as-judge confirmation pass are deferred to v2. See `specs/2026-05-11-phase-e1-missing-link-detection.md`. Sub-flags on `spandrel audit --semantic`: `--semantic-model <name>` (default: auto-detect from store), `--similarity-threshold <0..1>` (default `0.75`), `--max-candidates-per-node <int>` (default `5`). Sub-flags on `spandrel embed`: `--provider local|openai|ollama` (default `local`), `--model <name>`, `--yes` (skip the cost prompt that fires when estimated OpenAI cost ≥ $0.10; local and Ollama are free → no prompt).
...
- **`spandrel-author` skill** — write-time authoring discipline skill. Auto-loads when editing markdown with Spandrel frontmatter or auditing graph content. References canonical knowledge in `/patterns/authorship` and `/hypothesis` rather than hardcoding heuristics, so the skill stays current with the framework's own evolution of those patterns. Pairs with the audit CLI: cheap detectors flag low-signal content; the skill helps authors fix it.

- **Phase D-0 detector noise reduction** — calibration pass on the cheap detectors after the manual EA-OS / Definite audits surfaced 25–100% false-positive rates on three finding classes. Six suppression rules and sub-kind splits land in `src/audit/heuristics.ts` and `src/compiler/audit-pass.ts`: redundant `mentions` edges that duplicate already-described typed frontmatter edges, body Contents/TOC heading-aware suppression on tautologous edge descriptions, `topic_opening` suppression on legitimate collection-index descriptions, `stub_marker` split into framework-scaffold vs human-TODO sub-kinds, container-composite suppression on `thin_body` / `weak_description.thin`, and a docs sweep replacing stale `design.md` references with the post-0.6.0 `DESIGN.md` form. After D-0, audit findings on real graphs drop by ~70% with the same signal density preserved. See `specs/2026-05-11-quality-roadmap.md` § D-0.

- **Task-fidelity harness** — Phase D-1. Measures whether an MCP-consuming agent can do real tasks against a Spandrel graph. Lives at `test/fidelity/run.ts`. Spawns a `claude -p` subprocess per task with the graph's MCP server attached via `--mcp-config`, captures the tool-call log and final response, and scores via a combination of hard-gate substring matching and LLM-as-judge soft signals. Outputs structured JSON for diffing across runs. Ships with an EA-OS task set seed (`test/fixtures/task-sets/ea-os.json`, 10 curated questions) refined against a first baseline run (D-3 calibration: `ops` budget tuned 5→7; over-strict signal dropped). Empirical finding from the D-3 baseline that propagated into Phase E1 prioritization and the context-pack hygiene framing: `weak_edge_description` count does not correlate with task-fidelity outcomes on curated graphs. See `specs/2026-05-11-task-fidelity-harness.md` and `specs/2026-05-11-quality-roadmap.md` for the framing.

### Documentation

- **README rewritten to lead with the value prop.** Structure shifts from "Spandrel is a framework that..." (What) to Why / What / How / Get started. Headline framing: turning a markdown repository into a high-fidelity context graph served over MCP — so the agents reading it answer better, decide better, and create more faithfully to your actual structure. Architecture diagram added at `assets/architecture.png`, rendered from `scripts/render-architecture.ts` (new build script using `satori` + `@resvg/resvg-js`). The four-layer toolchain (compile / serve / govern / audit + measure) becomes the canonical narrative for the framework.
