...
- **Phase E1 — embedding infrastructure + missing-link detection.** New `spandrel embed` subcommand populates a per-graph SQLite cache of node embeddings (`<graph-root>/_audit/embeddings.db`, gitignored). New `spandrel audit --semantic` flag reads that cache and runs the missing-link detector — surfaces node pairs that are semantically close (cosine similarity ≥ 0.75 by default) but graph-distant (no `link`-type edge connects them; hierarchy edges don't count). Findings emit as a new `missing_link` warning type with messages of the form `[missing_link] Considered linking to <target> (cos 0.NN)`. Two embedding providers ship: OpenAI (`text-embedding-3-small`, default; requires `OPENAI_API_KEY`) and Ollama (`nomic-embed-text`, free/local; configurable via `OLLAMA_HOST`). The store is content-addressed by `(path, sha256(name+description+body), model)` so re-runs of `spandrel embed` are idempotent — only nodes whose content has changed get re-embedded. `spandrel audit --semantic` errors with a clear hint pointing at `spandrel embed` when the store is missing or stale. v1 emits symmetric candidates (both directions when both pass threshold); a suppression mechanism and LLM-as-judge confirmation pass are deferred to v2. See `specs/2026-05-11-phase-e1-missing-link-detection.md`. Sub-flags on `spandrel audit --semantic`: `--semantic-model <name>` (default `text-embedding-3-small`), `--similarity-threshold <0..1>` (default `0.75`), `--max-candidates-per-node <int>` (default `5`). Sub-flags on `spandrel embed`: `--provider openai|ollama` (default `openai`), `--model <name>`, `--yes` (skip the cost prompt that fires when estimated OpenAI cost ≥ $0.10).
