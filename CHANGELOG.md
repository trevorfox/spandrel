...
- **MCP tool responses strip null/undefined/empty-string fields at the wire boundary.** A single `stripNulls` pass at `src/server/mcp.ts` `asTextResult` walks every JSON tool response before serialization and omits keys whose value is `null`, `undefined`, or `""`. `false`, `0`, empty arrays, and empty objects are preserved (those carry meaning). Non-plain objects (`Date`, `Map`, class instances) pass through untouched so `JSON.stringify` can apply its own serialization rules. **REST API responses and TypeScript types are unchanged** — typed REST consumers (Cannon, downstream TS clients) keep their stable schema with explicit nulls; the hygiene rule is a wire-boundary transform on the MCP surface only, so MCP and REST emit different shapes for the same underlying data. Consumers reading parsed MCP JSON with `?? fallback` or `key in obj` are unaffected; consumers that read `field === null` as a structured signal would need to switch to absence checks. Calibration on three real graphs (Spandrel `docs/`, EA-OS, Definite): wire-size reduction is **0–12% per tool**, dominated by `navigate` and `get_node` (top-level metadata nulls — `parent`, `content`, `created`, `updated`, `author` — on nodes without git history). `get_references` and `get_graph` see ~0% on graphs where edge descriptions have been authored (the post-D-0 sweep filled most of them in); savings are larger on uncurated graphs. See `specs/2026-05-11-context-pack-hygiene.md`. Cannon coordination: zero — Cannon doesn't typed-consume MCP wire output internally.
