...
import { errorResponse, textResponse } from "../router.js";
 ...
 */
export const handleContent: RestHandler = async (_req, url, ctx) => {
  const nodePath = stripPrefix(url.pathname, "/content");
  const node = await ctx.store.getNode(nodePath);
  if (!node) return errorResponse(404, "not found");
  ...
  return textResponse(200, node.content, "text/markdown; charset=utf-8");
};
