 ...
 */
export const handleGetNode: RestHandler = async (_req, res, url, ctx) => {
  ...
  if (depth > MAX_DEPTH) {
    return sendError(res, 400, `depth exceeds maximum of ${MAX_DEPTH}`);
  }
  ...
  if (!shaped) return sendError(res, 404, "not found");
 ...
 */
export const handlePutNode: RestHandler = async (req, res, url, ctx) => {
  if (!ctx.rootDir) return sendError(res, 405, "writes not enabled");

  const nodePath = stripPrefix(url.pathname, "/node");

  if (!ctx.policy.canWrite(ctx.actor, nodePath)) {
    return sendError(res, 403, "write denied");
  }
  ...
  try {
    ...
    return sendError(res, 400, `invalid JSON body: ${(err as Error).message}`);
  }
 ...
 */
export const handleDeleteNode: RestHandler = async (_req, res, url, ctx) => {
  if (!ctx.rootDir) return sendError(res, 405, "writes not enabled");
  ...
  return sendJson(res, 200, { success: true, path: nodePath });
};
