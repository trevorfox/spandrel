    ...
    query: new GraphQLObjectType({
      ...
      fields: {
        ...
        children: {
          ...
          resolve: async (_root, args: { path: string; depth?: number; first?: number; after?: string }) => {
            const level = checkAccess(args.path);
            if (!accessLevelAtLeast(level, "description")) {
              return { nodes: [], pageInfo: { hasNextPage: false, endCursor: null } };
            }
            const allChildren = await resolveChildren(store, args.path, args.depth ?? 1);
            const accessible = await filterAccessible(allChildren);
        ...
        references: {
          ...
          resolve: async (_root, args: { path: string; direction?: string; first?: number; after?: string }) => {
            const level = checkAccess(args.path);
            if (!accessLevelAtLeast(level, "description")) {
              return { nodes: [], pageInfo: { hasNextPage: false, endCursor: null } };
            }
            const allRefs = await resolveReferences(store, args.path, args.direction ?? "outgoing");
            const accessible = await filterAccessible(allRefs);
            const { items, hasNextPage, endCursor } = paginateList(accessible, args.first, args.after);
            return { nodes: items, pageInfo: { hasNextPage, endCursor } };
          },
        },
        ...
        graph: {
          ...
          resolve: async (
            ...
            const result = await resolveGraph(store, args.path ?? "/", requestedDepth);
...
async function resolveGraph(
  ...
  while (toVisit.length > 0) {
    if (Date.now() > deadline) break; // query timeout guard
    const { path: p, d } = toVisit.pop()!;
    if (collectedNodes.has(p)) continue; // cycle detection
