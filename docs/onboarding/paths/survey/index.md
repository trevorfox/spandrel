...
- **Imported content produces many warnings — that's expected.** An existing directory with hundreds of `.md` files without Spandrel frontmatter will generate hundreds of `missing_name` / `missing_description` warnings on first compile. Do not try to fix all of these in the initial onboarding pass. The top-level shell (collection `index.md` files with good frontmatter) compiles cleanly; imported sub-tree content becomes legitimate follow-up work, fixed incrementally as files are edited. A run that turns up 1,000 warnings mostly in one or two client subtrees is normal for a mature existing corpus — don't let volume scare you or the agents into paralysis.
