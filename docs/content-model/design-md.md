...
**In knowledge repo collections:** A `design.md` in a collection like `/clients/` describes what a well-formed client Thing looks like — what frontmatter fields matter, what links to expect, what sub-Things it should have, what to avoid. It's the schema guidance that the bootstrap process and the intake pipeline read to know how to shape content for that part of the tree. When an agent is adding a new client, it reads the design doc to understand the conventions.
