...
- **0.10.0** — Quality infrastructure release. Adds the full advisory audit stack: Phase A cheap heuristics (`weak_description`, `weak_edge_description`, `stub_marker`, `thin_body`, `overlong_body`, `staleness`), Phase B audit-pass wiring + `spandrel audit` CLI subcommand, Phase C collection schemas (opt-in `schema:`/`graph:` on `DESIGN.md`, nine new schema-validation warnings) + `--priority` ranked queue, and Phase D-0 detector noise reduction (sub-kind splits + suppressions for redundant `mentions`, TOC tautologies, container composites, `topic_opening` on indexes, and framework-scaffold vs human-TODO stub markers). Phase E1 semantic infrastructure: `spandrel embed` subcommand + per-graph SQLite embedding cache, three providers (zero-setup local `fastembed`, opt-in `openai` and `ollama`), and `spandrel audit --semantic` flag with a new `missing_link` warning type. Phase D-1 task-fidelity harness at `test/fidelity/` for measuring real-agent performance against curated task sets. `spandrel-author` skill for write-time authoring discipline. **MCP wire-format change:** tool responses strip `null`/`undefined`/`""` fields at the `asTextResult` serialization boundary — REST API shape and TypeScript types unchanged, but MCP-consuming agents that read `field === null` as a structured signal must switch to absence checks. Top-level public imports unchanged.
